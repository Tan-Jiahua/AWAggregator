...
#' @param expectedRelativeAbundance A named list where group names are keys and the corresponding expected relative abundance values for species at varying concentrations are provided as values. Unknown ratios can be designated as NA.
#' @param speciesAtConstLevel A string specifying the species that are spiked in at a constant level.
...
#' samples <- c('H1+E1_1', 'H1+E1_2', 'H1+E1_3', 'H1+E2_1',
#'     'H1+E2_2', 'H1+E2_3', 'H1+E2_4', 'H1+E6_1', 'H1+E6_2', 'H1+E6_3')
#' groups <- c('H1+E1', 'H1+E1', 'H1+E1', 'H1+E2',
#'     'H1+E2', 'H1+E2', 'H1+E2', 'H1+E6', 'H1+E6', 'H1+E6')
...
getAvgScaledErrorOfLog2FC <- function(PSM, colOfReporterIonInt, groups, expectedRelativeAbundance, speciesAtConstLevel){

    PSM$Species <- str_match(PSM$Protein, '_([A-Z]+)$')[, 2]

    if(any(!is.na(expectedRelativeAbundance) & expectedRelativeAbundance < 0)){
        stop('All expected relative abundance provided by expectedRelativeAbundance should not be negative.')
    }
    if(any(!groups %in% names(expectedRelativeAbundance))){
        stop('The expected relative abundance of some groups is not defined.')
    }

    expectedRelativeAbundance <- expectedRelativeAbundance[expectedRelativeAbundance != 0 & !is.na(expectedRelativeAbundance)]
    ...
    for(idx in seq_len(ncol(cmps))){ # Get FC
        ...
        PSM[, FC] <-
            apply(PSM[, colOfReporterIonInt[groups == cmps[2, idx]]], MARGIN=1, FUN=mean) /
            apply(PSM[, colOfReporterIonInt[groups == cmps[1, idx]]], MARGIN=1, FUN=mean)
    }

    for(idx in seq_len(ncol(cmps))){ # Get Error of log2FC
        expected_FC <- abundance[2, idx] / abundance[1, idx]
        FC <- paste0('FC (', cmps[2, idx], ' vs ', cmps[1, idx], ')')
        PSM[, paste0('Error of log2', FC)] <- abs(log2(PSM[, FC]) - ifelse(PSM$Species == speciesAtConstLevel, 0, log2(expected_FC)))
    }
